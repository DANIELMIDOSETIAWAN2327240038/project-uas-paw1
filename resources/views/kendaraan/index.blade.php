            ...
            <tr>
              ...
              <td>
                ...
                <form action="{{ route('kendaraan.destroy', $item->id) }}" method="POST" class="d-inline">
                  @csrf
                  @method('DELETE')
                  <button type="submit" class="btn btn-danger show_confirm mt-2" data-lte-toggle="tooltip" title="Delete" data-nama="{{ $item->varian->merk->nama_merk }} {{ $item->varian->nama_tipe }} {{ $item->tipe_kendaraan }}"> Hapus </button>
                </form>
                @endcan
              </td>
            </tr>
